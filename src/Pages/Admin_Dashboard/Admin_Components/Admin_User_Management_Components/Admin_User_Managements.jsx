...
function Admin_User_Managements() {
  ...
  return (
    ...
    >
      ...
      <div className="h-fit flex items-center justify-between px-5 py-2 rounded-lg sticky top-0 bg-white z-30">
            ...
            <div className="relative border-2 border-gray-500 px-5 py-1 h-fit flex items-center gap-4 rounded-lg">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setStudentPurchaseData(studentPurchaseDataSample); // Reset to original data.

                  // Filter data based on search input.
                  studentPurchaseData?.map((item, idx) => {
                    const inputData = e.target.value.toLowerCase().trim();
                    console.log(inputData);
                    if (!e.target.value) {
                      setSearchFiltredData((prev) => []); // Clear filtered data if input is empty.
                      return;
                    }
                    if (item.email.toLowerCase().includes(inputData)) {
                      temp_data.push(item); // Add matching items to temporary data.
                    }
                    setSearchFiltredData((prev) => temp_data); // Update filtered data state.
                  });
                }}
              />
            </div>
              ...
              <div className="searchResultModal">
                {searchFiltredData.length > 0 ? (
                  <>
                    ...
                    >
                      {searchFiltredData?.map((item, idx) => {
                        return (
                          <div
                            className="relative cursor-pointer flex border-b items-center text-start gap-6 border-collapse py-2 px-1 border-gray-300 h-[65px] z-10"
                          >
                            ...
                            <div
                              ...
                              onClick={() => {
                                alert(JSON.stringify(item)); // Alert student data on click.
                              }}
                            >
        ...
        <div
          onClick={() => {
            ...
            console.log("Printing Is Filter Open....", isFilterOpen);
          }}
      ...
      {isFilterOpen && <Admin_User_Purchase_Filter setIsFilterOpen={setIsFilterOpen} />}
